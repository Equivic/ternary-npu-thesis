...
model_fp32 = MNISTNet()
model_fp32.load_state_dict(torch.load('trained_model.pt'))
model_fp32.eval()

# Create fp16 and bf16 copies
model_fp16 = MNISTNet()
model_fp16.load_state_dict(torch.load('trained_model.pt'))
model_fp16.half()

model_bf16 = MNISTNet()
model_bf16.load_state_dict(torch.load('trained_model.pt'))
model_bf16.bfloat16()

# Create INT8 model copy
model_int8 = MNISTNet()
model_int8.load_state_dict(torch.load('trained_model.pt'))
model_int8.eval()

# Create INT4 model copy
model_int4 = MNISTNet()
model_int4.load_state_dict(torch.load('trained_model.pt'))
model_int4.eval()
...
model_ternary_07.load_state_dict(torch.load('trained_model.pt'))
model_ternary_07.eval()

# Create Binary model copy
model_binary = MNISTNet()
model_binary.load_state_dict(torch.load('trained_model.pt'))
model_binary.eval()
...
models = [model_fp32, model_fp16, model_bf16, model_int8, model_int4, model_ternary, model_ternary_07, model_binary]
names = ['fp32', 'fp16', 'bf16', 'int8', 'int4', 'ternary', 'ternary_0.7', 'binary']

fig, axes = plt.subplots(len(models), len(neuron_picks), figsize=(len(neuron_picks) * 1.3, len(models) * 1.5))
